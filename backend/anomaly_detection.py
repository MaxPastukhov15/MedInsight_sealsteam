...
def _validate_anomaly_dataframe(df: pd.DataFrame, required_columns: List[str]) -> Tuple[pd.DataFrame, List[str]]:
    """
    ...
    if missing:
        raise ForecastError(f"Missing columns: {missing}. Got: {list(df.columns)}")
...
def detect_spatial_anomalies(
    ...
    if len(df) < original_len:
        warnings_list.append(f"Dropped {original_len - len(df)} rows with invalid values")

    if len(df) < 3:
        raise ForecastError(f"Need at least 3 categories for outlier detection, got {len(df)}")
