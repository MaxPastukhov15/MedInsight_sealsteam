...
def analyze_trend_pipeline(df_subset):
    ...
    for m in range(1, 13):
        ...
        seasonality_data.append({"month": int(m), "avg_cases_per_week": round(val, 2), "status": status})
    ...
    for date_idx, val in ts.items():
        ...
        timeline.append({"date": str(date_idx.date()), "cases": int(val), "rolling_mean": round(float(r_val), 2)})
...
def get_demographic_insights(df):
    ...
    """
    stats = (
        df.groupby(["root_code", "root_name"])
        .agg(
            {
                "age": "mean",
    ...
    insights = {"elderly_diseases": [], "youth_diseases": [], "female_diseases": [], "male_diseases": []}
